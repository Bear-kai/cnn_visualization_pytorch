...
Mobilefacenet_setting_air = [
    # t, c , n ,s
    [2, 64, 10, 2],                          # 10=1+9, 后面4个块输入输出都是28x28
    [4, 128, 1, 2],
    [2, 128, 16, 1],
    [8, 256, 1, 2],
    [2, 256, 6, 1]
...
class MobileFaceNet(nn.Module):
    def __init__(self, setting_str='MobileFaceNet', CBAM=False, tanh_act=False, allow_dim_2x=True):
        ...
        self.linear7 = ConvBlock(512, 512, (7, 7), 1, 0, dw=True, linear=True)  # linear GDConv7x7; 若输入img_size是112*96，则此处kernel_size可改为7*6

        self.linear1 = ConvBlock(512, 128, 1, 1, 0, linear=True)  # linear conv1x1
        if bottleneck_setting[2][2] != 6 and allow_dim_2x:
            self.linear1 = ConvBlock(512, 256, 1, 1, 0, linear=True)   # 对应MobileFaceNet_2x, 输出特征维度加倍
...
class MobileFaceNet_air(nn.Module):
    def __init__(self, bottleneck_setting=Mobilefacenet_setting_air, CBAM=False, tanh_act=False):
        ...
        self.linear1 = ConvBlock(1024, 128, 1, 1, 0, linear=True)  # linear conv1x1 论文的Table 1中是输出512dim，实验中描述是128dim
